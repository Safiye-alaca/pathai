...
export default function RadarPanel({}: RadarPanelProps) {
  ...
  // Varsayılan durum metnini dile göre başlatıyoruz
  const [statusText, setStatusText] = useState(t.radarPanel.desc);
  const wsRef = useRef<WebSocket | null>(null);

  // Dil değiştiğinde eğer tarama başlamadıysa durum açıklamasını güncelle
  useEffect(() => {
    if (!isLoading && githubItems.length === 0) {
      setStatusText(t.radarPanel.desc);
    }
  }, [t, isLoading, githubItems.length]);
  ...
  const startWebSocketScan = () => {
    ...
    setSummaryData(null);      
    setIsLoading(true);
    setStatusText(language === "tr" ? "📡 Canlı WebSocket tüneli açılıyor..." : "📡 Opening live WebSocket tunnel...");
    ...
    ws.onmessage = (event) => {
      ...
      if (data.status === "COMPLETED") {
        setStatusText(language === "tr" ? "✅ Tüm küresel yapay zeka kanallarının taraması başarıyla tamamlandı." : "✅ Scanning of all global AI channels successfully completed.");
        setIsLoading(false);
        setIsScanCompleted(true); 
        ws.close();
      } else if (data.status === "ERROR") {
        setStatusText(language === "tr" ? `❌ Tarama hatası: ${data.message}` : `❌ Scan error: ${data.message}`);
        setIsLoading(false);
        ws.close();
      } else {
    ...
    ws.onerror = (err) => {
      ...
      setStatusText(language === "tr" ? "❌ Sunucu ile canlı bağlantı kurulamadı." : "❌ Could not establish live connection with server.");
      setIsLoading(false);
    };
  };
  ...
  const generateAIReport = async () => {
    ...
    try {
      ...
      if (!res.ok) throw new Error(language === "tr" ? "Yapay zeka rapor motoru şu an yanıt vermiyor." : "AI report engine is not responding right now.");
      const data = await res.json();
      setSummaryData(data);
    } catch (err: any) {
      console.error(err);
      setStatusText(language === "tr" ? `❌ Özetleme Hatası: ${err.message}` : `❌ Summarization Error: ${err.message}`);
    } finally {
    ...
    <div className="space-y-8 animate-fade-in">
      ...
      <div className="bg-white border border-purple-100 rounded-[32px] p-6 flex flex-col md:flex-row items-center gap-6 shadow-xl max-w-3xl mx-auto">
        ...
        <div className="text-center md:text-left space-y-2 flex-1">
          <h1 className="text-2xl font-black text-purple-950">{t.radarPanel.title}</h1>
          ...
          <div className="flex flex-col sm:flex-row sm:items-center gap-4 pt-1 justify-center md:justify-start">
            ...
            >
              {isLoading ? (
                <>
                  ...
                  {t.radarPanel.loading}
                </>
              ) : (
                language === "tr" ? "🔍 Ağları Canlı Tara" : "🔍 Scan Networks Live"
              )}
            </button>

            {/* Sekme (Tab) Değiştirici Alanı */}
            <div className="inline-flex bg-slate-100 p-1 rounded-full border border-slate-200/60 self-center">
              ...
              >
                {language === "tr" ? "🛠️ Kod & Model" : "🛠️ Code & Model"}
              </button>
              <button
              ...
              >
                {language === "tr" ? "📰 Girişimler & Haberler" : "📰 Startups & News"}
              </button>
            </div>
          </div>
        </div>
      ...
      {/* Tarama Bittiyse Ortaya Çıkan Akıllı Özet Butonu */}
      {isScanCompleted && (
        <div className="text-center bg-gradient-to-r from-purple-900 to-indigo-950 p-6 rounded-3xl shadow-xl text-white space-y-3 animate-slide-up">
          <div className="text-xs text-purple-200 font-bold">
            {language === "tr" ? "🧠 Küresel AI verileri başarıyla hafızaya alındı!" : "🧠 Global AI data successfully memorized!"}
          </div>
          <p className="text-[11px] opacity-80 max-w-md mx-auto">
            {language === "tr" 
              ? "Akan tüm repoları, finansman haberlerini ve teknik tartışmaları tek tıkla analiz edip günün pazar raporunu çıkartın."
              : "Analyze all streaming repos, funding news, and technical discussions with one click to generate the market report of the day."}
          </p>
          <button
          ...
          >
            {summaryLoading 
              ? (language === "tr" ? "Gemini Verileri Yorumluyor..." : "Gemini is Interpreting Data...") 
              : (language === "tr" ? "✨ Ajan Özetini ve Pazar Analizini Al" : "✨ Get Agent Summary & Market Analysis")}
          </button>
        </div>
      )}

      {/* Akıllı Özet Sonuç Raporu Ekranı */}
      {summaryData && (
          ...
          <div className="border-b pb-3 border-amber-100 flex items-center gap-2">
            ...
            <div>
              <h3 className="font-black text-purple-950 text-base">
                {language === "tr" ? "Gemini Yapay Zeka Strateji Raporu (TL;DR)" : "Gemini AI Strategy Report (TL;DR)"}
              </h3>
              <p className="text-slate-400 text-[10px]">
                {language === "tr" ? "Canlı taranan verilerden çıkarılan anlık küresel özet." : "Real-time global summary extracted from live scanned data."}
              </p>
            </div>
          </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            <div className="bg-purple-50/40 border border-purple-100 p-4 rounded-2xl space-y-2">
              <h4 className="font-bold text-purple-950 text-xs flex items-center gap-1">
                🎯 {language === "tr" ? "Yakalanan Pazar Fırsatları" : "Captured Market Opportunities"}
              </h4>
            ...
            <div className="bg-indigo-50/40 border border-indigo-100 p-4 rounded-2xl space-y-2">
              <h4 className="font-bold text-indigo-950 text-xs flex items-center gap-1">
                🚀 {language === "tr" ? "Yükselen Mimari Trendler" : "Rising Architectural Trends"}
              </h4>
              <ul className="space-y-1.5 pl-1">
      ...
      {isLoading && githubItems.length === 0 && techcrunchItems.length === 0 && (
        <div className="text-center py-12">
          ...
          <p className="text-slate-500 text-xs font-semibold">
            {language === "tr" ? "Küresel API hatları üzerinden veri akışı başlatılıyor..." : "Initiating data flow across global API lines..."}
          </p>
        </div>
      )}
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
          ...
          <div className="bg-white border border-purple-100 rounded-[28px] p-6 shadow-xl space-y-4">
            <h2 className="text-base font-black text-purple-950 flex items-center gap-2 border-b pb-2 border-purple-50">
              🐙 {language === "tr" ? "Trend Repolar (GitHub)" : "Trending Repos (GitHub)"}
            </h2>
          ...
          <div className="bg-white border border-purple-100 rounded-[28px] p-6 shadow-xl space-y-4">
            <h2 className="text-base font-black text-purple-950 flex items-center gap-2 border-b pb-2 border-purple-50">
              🤗 {language === "tr" ? "En Popüler Modeller (Hugging Face)" : "Most Popular Models (Hugging Face)"}
            </h2>
            <div className="space-y-3 max-h-[450px] overflow-y-auto pr-1">
      ...
      {activeTab === "news" && (techcrunchItems.length > 0 || hackernewsItems.length > 0) && (
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
          ...
          <div className="bg-white border border-purple-100 rounded-[28px] p-6 shadow-xl space-y-4">
            <h2 className="text-base font-black text-purple-950 flex items-center gap-2 border-b pb-2 border-purple-50">
              🚀 {language === "tr" ? "Lansman & Yatırımlar (TechCrunch)" : "Launches & Funding (TechCrunch)"}
            </h2>
            <div className="space-y-3 max-h-[450px] overflow-y-auto pr-1">
          ...
          <div className="bg-white border border-purple-100 rounded-[28px] p-6 shadow-xl space-y-4">
            <h2 className="text-base font-black text-purple-950 flex items-center gap-2 border-b pb-2 border-purple-50">
              🔥 {language === "tr" ? "Derin Teknik Tartışmalar (Hacker News)" : "Deep Technical Discussions (Hacker News)"}
            </h2>
            <div className="space-y-3 max-h-[450px] overflow-y-auto pr-1">
